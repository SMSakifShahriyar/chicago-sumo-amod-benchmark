...
scales = [0.006, 0.008, 0.010, 0.012, 0.015]

results_csv = os.path.join(project_dir, "output", "persistent_A_lowload_results.csv")
summary_txt = os.path.join(project_dir, "output", "persistent_A_lowload_summary.txt")
...
def run_one(scale):
    ...
    prefix = f"persistent_A_lowload_{scale_tag}"
...
def main():
    ...
    for scale in scales:
        ...
        rows.append(row)
        print(
            f"done scale={scale} total={row['total_requests']} assigned={row['requests_assigned']} "
    ...
    if valid_rows:
        ranked = sorted(
            valid_rows,
            key=lambda r: (r["served_fraction"], -r["teleports"], -r["collisions"], r["requests_served"]),
            reverse=True,
        )
        best = ranked[0]

    lines = []
    lines.append("persistent A low-load sweep summary")
    lines.append("")
    ...
    if best is not None:
        ...
        lines.append(f"requests_assigned={best['requests_assigned']}")
        lines.append(f"requests_served={best['requests_served']}")
        lines.append(f"requests_waiting_end={best['requests_waiting_end']}")
